...
import { computeArTotals } from "@/types/associationReceipt"
...
export function SavedAssociationReceiptCard({ saved, onDuplicate, onLoad, onDelete, onRename }: Props) {
  ...
  const { totalTTC } = computeArTotals(saved.receipt.items)
      ...
      <CardContent className="p-5 space-y-3">
        ...
        <div className="space-y-0.5 text-sm text-muted-foreground">
          <p className="font-medium text-foreground truncate">
            {saved.receipt.requester.name || "—"}
          </p>
          <p className="text-xs">{saved.receipt.requester.role || ""}</p>
          <p>{formatDate(saved.receipt.date)}</p>
          <p className="text-base font-semibold text-foreground pt-0.5">{fmt.format(totalTTC)}</p>
          <p className="text-xs">{saved.receipt.items.length} dépense{saved.receipt.items.length > 1 ? "s" : ""}</p>
        </div>

        {saved.usageCount > 0 && (
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            <TrendingUp className="size-3" />
            <span>Utilisé {saved.usageCount} fois</span>
          </div>
        )}
