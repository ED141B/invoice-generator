...
import { computeExpenseTotals } from "@/types/expense"
...
export function SavedExpenseCard({ saved, onDuplicate, onLoad, onDelete, onRename }: Props) {
  ...
  const { totalTTC } = computeExpenseTotals(saved.expense.items)
      ...
      <CardContent className="p-5 space-y-3">
        ...
        <div className="space-y-0.5 text-sm text-muted-foreground">
          <p className="font-medium text-foreground truncate">
            {saved.expense.employee.name || "—"}
          </p>
          <p className="text-xs">{saved.expense.employee.role || ""}</p>
          <p>{formatDate(saved.expense.periodStart)}</p>
          <p className="text-base font-semibold text-foreground pt-0.5">{fmt.format(totalTTC)}</p>
          <p className="text-xs">{saved.expense.items.length} dépense{saved.expense.items.length > 1 ? "s" : ""}</p>
        </div>
